# Studio AZ Tattoo — SEO Playbook

> Synthesized from The Affiliate Lab (Matt Diggity), 2026 local SEO research, GBP ranking studies, GEO best practices, and competitor analysis. This is the single source of truth for all SEO decisions on tattooshopminneapolis.com.
...
**Our biggest lever is Prominence.** Distance is fixed. Relevance is handled by proper category selection and on-page SEO. But we can massively grow Prominence through reviews, content, citations, and GBP optimization.

### 1.2 Google Business Profile (GBP) Optimization

GBP signals account for ~32% of all map pack ranking factors. This is the single most impactful thing for local search.

**Foundation:**
- [ ] Business name matches real-world branding exactly — no keyword stuffing (August 2025 Spam Update penalizes this)
- [ ] Primary category = "Tattoo Shop" (matches core revenue service)
...
**Content & Engagement:**
- [ ] Business description: natural language, clearly states what we do and where, services mentioned once, updated annually
...
- [ ] Short video clips: studio tour, tattoo process timelapse, artist intros
...
Reviews are the #1 lever for Map Pack visibility. We have 9 reviews vs market leader average of ~350 (39x gap).
...
- Prompt clients to mention specific services naturally ("How was your fine line tattoo experience?")
- Customer mentions of "kitchen remodel" or "fine line tattoo" create "Review Justification" signals in Google
...
2. Yelp (claim listing — currently unclaimed)
...
8. Minneapolis Chamber of Commerce
9. Industry-specific: TattooCloud, Tattoodo
...
- Neighborhood association pages (North Loop Business Association)
...
- Do not repeat words (e.g., `tattooshopminneapolis.com/tattoo-services` repeats "tattoo" — bad)
- Get the frequently repeating SINGLE WORDS from top keywords into the slug
- Our domain `tattooshopminneapolis.com` IS our #1 keyword — biggest SEO asset. No competitor has this.
...
Every important page must be reachable within 2-3 clicks of the homepage. Our 10-page site naturally achieves this.
...
**Navigation:** Primary nav should have 7±2 items (cognitive load research). Our 7 main nav items (Home, Services, Artists, Gallery, Aftercare, FAQ, Contact) are ideal. Parking can be in footer or under Contact.
...
- Brand/URL ("Studio AZ," "tattooshopminneapolis.com"): 25%
...
- Cross-link related pages: Services ↔ Artists, Aftercare ↔ FAQ, Artists ↔ Gallery
- Don't link unrelated topics — don't confuse Google's topic understanding

**Breadcrumbs:**
- BreadcrumbList schema on all non-homepage pages
- Example: Home > Artists > Joan Martinez
...
- Our target: 7,000-8,500 total words across 10 pages (more than any competitor's entire site)
- Homepage: 1,500-2,000 words (Nokomis leads market with 2,500)
- Service pages: 800-1,000 words
- Artist pages: 400-500 words each + portfolio
- FAQ: 1,500-2,000 words (15-20 questions)
- Aftercare: 1,000-1,500 words
...
- Dofollow
- Cover the entire niche (e.g., link to Wikipedia for "Minneapolis," link to AAD.org for aftercare science)
...
**Metadata:**
- Use `generateMetadata()` for dynamic pages (artist pages, etc.)
...
- Include all 10 pages with `lastModified` dates and `changeFrequency`
...
- See Schema section below for per-page schemas

### 3.3 Image Optimization

- All images in WebP format with JPEG fallbacks
- Use `next/image` for automatic responsive sizing and lazy loading
- File names: keyword-descriptive (e.g., `fine-line-tattoo-minneapolis-joan-martinez.webp`)
...
| Page | Schema Types |
|------|-------------|
| Home | `TattooParlor` (extends LocalBusiness), `Organization`, `BreadcrumbList` |
| Services | `Service` + `Offer` (per service), `BreadcrumbList` |
| Artists (index) | `ItemList` of `Person` entities, `BreadcrumbList` |
| Joan / Andrew | `Person` with `worksFor`, `knowsAbout`, portfolio `ImageGallery`, `BreadcrumbList` |
| Gallery | `ImageGallery` + `ImageObject` per image, `BreadcrumbList` |
| Aftercare | `Article` + `HowTo` (step-by-step healing), `BreadcrumbList` |
| Parking | `Article`, `BreadcrumbList` |
| FAQ | `FAQPage` (every Q&A = a featured snippet opportunity), `BreadcrumbList` |
| Contact | `TattooParlor` (repeated for NAP consistency), `BreadcrumbList` |
...
- Include `AggregateRating` on homepage once we have 10+ reviews
...
- Process documentation: "Here's how we create custom tattoos"
- Artist bios with years of experience, training, specialties
- Client testimonials with specific details ("Joan did my sleeve over 3 sessions...")
- Before/after gallery (where appropriate)

### 5.2 Expertise
- Artist pages with detailed specialization (fine line, realism, black & grey)
- Aftercare page written with medical/dermatological accuracy
- FAQ answers that demonstrate deep industry knowledge
- Link to relevant medical/health authority pages (AAD.org for skin care)
...
- Instagram following and engagement as social proof

### 5.4 Trustworthiness
- HTTPS (mandatory)
- Clear NAP on every page (footer)
- Real photos of studio, team, and work
...
- Consistent brand mentions: "Studio AZ Tattoo" — same everywhere
- Detailed About page with team bios and business history
- Pursue third-party coverage: local blogs, Minneapolis magazines, tattoo publications
- Brand demand (people searching "Studio AZ Tattoo" directly) is a credibility signal
- AI engines favor earned media over brand-owned content for citations

### 6.4 Measuring AI Visibility

Track quarterly:
- AI citation frequency: how often ChatGPT/Perplexity/Gemini mention Studio AZ
...
2. Match the structure of the current snippet — if it's a list, use a list. If it's a table, use a table. If it's a paragraph, use a paragraph
3. Precede the answer with an H2 that contains the query (e.g., `<h2>How Long Does a Tattoo Take to Heal?</h2>`)
...
- Aftercare page (healing timeline, care instructions)
...
- Example: Query: "How much does a tattoo cost in Minneapolis?" → Answer: "The average tattoo in Minneapolis costs between $150-$300 for small pieces and $200+ per hour for larger custom work at most shops."
...
- Identify niche-specific terms we're missing (e.g., "consultation," "custom design," "walk-in," "appointment")
...
- **Early stage** (informational): "does getting a tattoo hurt," "tattoo aftercare" → educate, don't pitch. Soft CTA: "See Our Artists"
- **Mid stage** (consideration): "best tattoo shop Minneapolis" → show proof, reviews, portfolio. CTA: "View Our Artists"
- **Late stage** (ready to act): artist pages, services → direct CTA: "Book a Consultation"
...
- Author schema and bio at bottom of every content page (except Contact, FAQ utility pages)
- Link author bio to About page + social profiles
...
- Search for "tattoo shop Minneapolis" on ChatGPT, Perplexity, and Gemini
...
### 12.3 Social Signals
- Instagram presence (primary for tattoo industry — visual portfolio)
- Google Maps link from Instagram bio
...
What moves the needle most for a local tattoo shop, in order:

1. **Reviews** (40%) — 9 → 50+ unlocks map pack visibility
...
Our domain `tattooshopminneapolis.com` is the strongest SEO asset in the Minneapolis tattoo market. No competitor has their primary keyword in their domain. Combined with 2,500+ words of properly optimized content, comprehensive schema, and aggressive review generation, we can realistically achieve top-3 map pack placement within 6-9 months.
