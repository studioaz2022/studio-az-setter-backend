/**
 * Simulate incoming FAQ message webhook to trigger AI Bot response for qualified lead
 * This will make the AI bot respond with userId: 3dsbsgZpCWrDYCFPvhKu
 * and message ending with "-FrontDesk"
 */
...
// Simulate a GHL message webhook payload with FAQ question
const simulatedWebhook = {
  ...
  message: 'What time is my appointment?', // FAQ question to trigger response
  type: 'SMS',
  direction: 'inbound',
  phone: '+18329390214',
  locationId: GHL_LOCATION_ID,
...
async function simulateIncomingFAQ() {
  console.log('🧪 SIMULATING FAQ MESSAGE FROM QUALIFIED LEAD\n');
  ...
  try {
    ...
    if (webhookResponse.data.skipped) {
      ...
      console.log('\nThis is unexpected for an FAQ question.');
    ...
    console.log('Message preview:', latestOutbound.body?.substring(0, 150) + '...');
    ...
    if (latestOutbound.userId === AI_BOT_USER_ID) {
      ...
      if (latestOutbound.body?.includes('-FrontDesk')) {
        console.log('✅ Message ends with "-FrontDesk" (qualified lead response)');
      } else {
        console.log('⚠️  Message does NOT end with "-FrontDesk"');
        console.log('   This might be a bug - qualified leads should have this suffix');
      }
